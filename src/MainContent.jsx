...
const MainContent = ({ cartCount, setCartCount }) => {
  ...
  return (
    <section className="max-w-xl flex flex-col gap-5 text-center sm:text-left px-4 sm:px-0">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4">
        <div className="flex items-center justify-between bg-gray-200 w-60 h-14 rounded-lg">
          <button onClick={() => setCount((c) => Math.max(c - 1, 0))} className="text-amber-600 text-2xl font-bold px-3 cursor-pointer">-</button>
          <span className="text-lg font-bold text-blue-700">{count}</span>
          <button onClick={() => setCount((c) => c + 1)} className="text-amber-600 text-2xl font-bold px-3 cursor-pointer">+</button>
        </div>
        <button
          onClick={handleAdd}
